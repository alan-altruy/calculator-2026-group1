package calculator.operations.unaryoperations;

import calculator.Expression;
import calculator.enums.Notation;
import calculator.exceptions.IllegalConstruction;
import calculator.value.Value;
import java.util.List;

/**
 * Class representing the absolute value operation.
 */
public class Abs extends UnaryOperation {

    /**
     * Constructor for Abs operation.
     * @param elist List of expressions (should contain exactly one expression for unary operation)
     * @throws IllegalConstruction if the number of expressions is not exactly one
     */
    public Abs(List<Expression> elist) throws IllegalConstruction { super(elist); symbol = "|x|"; }

    /**
     * Constructor for Abs operation with specified notation.
     * @param elist List of expressions (should contain exactly one expression for unary operation)
     * @param n Notation to be used for this operation
     * @throws IllegalConstruction if the number of expressions is not exactly one
     */
    public Abs(List<Expression> elist, Notation n) throws IllegalConstruction { super(elist, n); symbol = "|x|"; }


    /**
     * Applies the absolute value operation to the given value.
     * @param v The value to which the absolute value operation will be applied
     * @return The result of applying the absolute value operation to the given value
     */
    @Override
    public Value unOp(Value v) { return v.abs(); }

    /**
     * Returns the precedence of the absolute value operation.
     * @return The precedence level of the absolute value operation
     */
    @Override
    public int getPrecedence() { return 4; }
}
